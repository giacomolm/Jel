define("raphaelext", ['raphael'], function(){
	Raphael.fn.connection = function (id, obj1, obj2, line, bg, context) {
		...
		obj.tri.contextmenu(function(event){
			obj.menu.toFront();// the context menu must be over other canvas elements
			obj.menu.show(event.layerX,event.layerY);
			event.preventDefault(true);
			event.stopImmediatePropagation();
		});

		obj.menu = this.menu(["removeArrow"],context, obj);
	...
	Raphael.fn.shape = function(url, x, y, width, height, context, definition){
		...
		shapeEl.menu = this.menu(["removeShape", "addArrow", "removeArrow"], context, shapeEl);
		shapeEl.contextmenu(function(event){
			shapeEl.menu.toFront();// the context menu must be over other canvas elements
	...
	Raphael.fn.menu = function (itemList, context, obj){
		...
		this.box = raphael.rect(0,0,100,25*itemList.length);
		this.box.attr({fill:'#EEE'});
		this.el.push(this.box);

		var i;
		for(i=0; i<itemList.length; i++){
			var boxtext = raphael.text(0,20,itemList[i]);
			boxtext.type = 'text'; 
			boxtext.name = itemList[i];
			...
			boxtext.click(function(){
							...
							$(obj).trigger(boxtext.name, [obj.id]);
						}
			);
			this.el.push(boxtext);
		}
